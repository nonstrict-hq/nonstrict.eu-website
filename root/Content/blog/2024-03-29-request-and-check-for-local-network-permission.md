...
slug: request-and-check-for-local-network-permission
title: Request and check local network permission on iOS and visionOS
description: The local network permission has, in contrast to other permissions, no
  simple way to request the permission or check its autorization state. We use `NWBrowser`
  to roll our own permission helper.
authors:
- mathijs
- tom
tags:
